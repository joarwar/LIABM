...
void uart_Init(void)
{
  ...
  stLink_Uart.Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
  stLink_Uart.Init.ClockPrescaler = UART_PRESCALER_DIV1;
  stLink_Uart.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
...
{
  HAL_UART_Transmit(&stLink_Uart, (uint8_t *)str, strlen(str), HAL_MAX_DELAY);
}
